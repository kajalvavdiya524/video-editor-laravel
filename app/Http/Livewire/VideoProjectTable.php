...
/**
 * Class VideoProjectTable.
 */
class VideoProjectTable extends TableComponent
{
    /**
     * @var string
     */
    ...
    {
        $query = VideoProject::with('user')->where('is_draft', false);
        $user = auth()->user();
        if ($user->isMasterAdmin()) return $query;

        if ($user->isTeamMember()) {
            $query = VideoProject::with('user')->where('visibility', true)->groupBy('id')->where('is_draft', false)
    ...
    {
        $active_columns = auth()->user()->getVideoProjectColumns();
        $columns = [];
