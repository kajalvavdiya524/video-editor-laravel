...
this.buildFileInfoCard = function (data) {
    ...
    document.getElementById("file-div-created-at").innerText =
        data["createdAt"];
    document.getElementById("file-div-updated-at").innerText =
        data["updatedAt"];
};
...
this.clickFile = function (e) {
    axios
        .get(
            "/admin/auth/video/media/file?id=" +
                e.target.getAttribute("atr") +
                "&thisFolder=" +
                document.getElementById("this-folder-id").value
        )
...
this.folderChangeName = function (e) {
    axios
        .get(
            "/admin/auth/video/media/folder?id=" + e.target.getAttribute("atr")
        )
        .then(function (response) {
...
this.moveFile = function (e) {
    document.getElementById("file-move-file-id").value =
        e.target.getAttribute("atr");
    self.showCard("file-move-file");
};

this.moveFolder = function (e) {
    document.getElementById("file-move-folder-id").value =
        e.target.getAttribute("atr");
...
this.deleteFolder = function (e) {
    $("#remove-folder-modal").modal("show");
    document.getElementById("file-delete-folder-id").value =
        e.target.getAttribute("atr");
};

this.deleteFile = function (e) {
    const curNode = e.target;
    let attr = curNode.getAttribute("atr");
    if (attr == null) {
        attr = curNode.parentNode.getAttribute("atr");
    }
    if (attr) {
        $("#file-delete-file-id").val(attr);
...
document
    .getElementById("file-rename-file-cancel")
    .addEventListener("click", this.renameFileCancel);
document
    .getElementById("file-rename-folder-cancel")
    .addEventListener("click", this.renameFolderCancel);
document
    .getElementById("file-move-file-cancel")
    .addEventListener("click", this.moveFileCancel);
document
    .getElementById("file-move-folder-cancel")
    .addEventListener("click", this.moveFolderCancel);
