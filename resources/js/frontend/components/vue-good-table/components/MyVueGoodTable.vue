    ...
    <div class="vgt-inner-wrap" :class="{ 'is-loading': isLoading }">
      ...
      <div :class="{ 'vgt-responsive': responsive }" :style="wrapperStyles">
        <table id="vgt-table" ref="table" :class="tableStyles">
          ...
          >
            <template slot="line-number-header">
              <add-scene-dropdown :addSceneCallback="addSceneCallback" :addSceneUsersCallback="addSceneUsersCallback"></add-scene-dropdown>
            </template>
...
import AddSceneDropdown from "../../common/AddSceneDropdown";
...
export default {
  ...
  methods: {
    ...
    togglecolspan(toggle){
      this.filteredRows.forEach((row) => {
      this.$set(row, "vgtIsExpanded", toggle);
        if(toggle==true){
