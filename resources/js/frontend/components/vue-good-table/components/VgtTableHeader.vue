...
<thead>
  <tr>
    <th scope="col" v-if="lineNumbers" class="text-left fixed-width">
      <slot name="line-number-header"></slot>
    </th>
    ...
    <th v-for="(column, index) in columns"
      ...
      v-if="!column.hidden"
      @click="toggletbl(column.label)">
      <slot name="table-column" :column="column">
        {{column.label}}
      </slot>

      <slot v-if="column.label=='Actions'">
          <span id="actions" class="triangle" :class="{'expand':checktoggle}"></span>
      </slot>
        <button
...
export default {
  ...
  methods: {
    toggletbl(chk){
      ...
      this.$emit('filter-toggle', this.checktoggle);
      }
    },
    reset() {
...
.vgt-table .triangle.expand:after {
